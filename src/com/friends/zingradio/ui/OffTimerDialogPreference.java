...
public class OffTimerDialogPreference extends DialogPreference implements TimePicker.OnTimeChangedListener
{
    public static final String TAG = OffTimerDialogPreference.class.getSimpleName();
    public static final String TIMER_OFF_HOURS_KEY = "TIMER_OFF_HOUR_KEY";
    public static final String TIMER_OFF_MINUTES_KEY = "TIMER_OFF_MINUTES_KEY";
    private Calendar calendar;
    private TimePicker picker = null;
    private int mSelectedHours;
    private int mSelectedMinutes;

    public OffTimerDialogPreference(Context ctx, AttributeSet attrs)
    {
        super(ctx, attrs);
        ...
        SharedPreferences sharedPref = this.getContext().getSharedPreferences(SettingsActivity.SHARED_PREF_USE_TIMER_OFF, Context.MODE_MULTI_PROCESS);
        boolean useTimer = sharedPref.getBoolean(SettingsActivity.SHARED_PREF_USE_TIMER_OFF_KEY, false);
        mSelectedHours = sharedPref.getInt(TIMER_OFF_HOURS_KEY, 0);
        mSelectedMinutes = sharedPref.getInt(TIMER_OFF_MINUTES_KEY, 0);
    ...
    protected void onDialogClosed(boolean positiveResult)
    {
        ...
        if (positiveResult)
        {
            ...
            editor.clear();
            editor.putBoolean(SettingsActivity.SHARED_PREF_USE_TIMER_OFF_KEY, useOffTimer);//need to rewrite otherwise it will be default to false. Android bug?
            editor.putInt(TIMER_OFF_HOURS_KEY, mSelectedHours);
            editor.putInt(TIMER_OFF_MINUTES_KEY, mSelectedMinutes);
            editor.commit();
